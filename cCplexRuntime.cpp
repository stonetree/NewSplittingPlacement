		...
		for (k=0;k<m_SizeK;k++)
		{
			for (h=0;h<m_SizeH;h++)					
			{
				for (j=0;j<m_SizeJ;j++)
				{
					...
					{
						_request.setSVMNumber(k+1);
						_request.setSVMResRequest((resourceRequirement[_request.getOriginalResRequest()])(_request.getOriginalResRequest(),_request.getLambda(),k+1));
						_request.host_server_vec.push_back(&(_server_vec[j]));
						...
						(_server_vec[j]).setTimeResourceUsed(_request,(resourceRequirement[_request.getOriginalResRequest()])(_request.getOriginalResRequest(),_request.getLambda(),k+1));
						(_server_vec[j]).setTimeWeight(_request,(resourceRequirement[_request.getOriginalResRequest()])(_request.getOriginalResRequest(),_request.getLambda(),k+1));
